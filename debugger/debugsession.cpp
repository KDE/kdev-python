...
DebugSession::DebugSession(QString interpreter, QStringList program, const QUrl& workingDirectory,
    ...
    , m_locationUpdate([this](const ResponseData& d) {
        locationUpdateReady(d);
    })
{
...
void DebugSession::start()
{
    ...
    m_debugger->setMethod(m_debugger->currentSeqnro(), m_locationUpdate);
...
void DebugSession::jumpToCursor()
{
    ...
    if (KDevelop::IDocument* doc = KDevelop::ICore::self()->documentController()->activeDocument()) {
        ...
        if ( cursor.isValid() ) {
            ...
            m_debugger->request(QStringLiteral("jump %1").arg(cursor.line() + 1), m_locationUpdate);
        }
    }
}
...
void DebugSession::locationUpdateReady(const ResponseData& data)
{
