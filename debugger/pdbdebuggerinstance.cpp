...
void PdbDebuggerInstance::request(QString command, CmdCallback callback)
{
    // Assemble the request body.
    QJsonObject cmd;
    cmd.insert(QStringLiteral("seq"), QJsonValue(m_currentSeqnro));
    cmd.insert(QStringLiteral("input"), QJsonValue(command + QStringLiteral("\n")));

    // Register the callback object.
    setMethod(m_currentSeqnro, callback);
    qCDebug(KDEV_PYTHON_DEBUGGER).noquote() << "seqnro" << m_currentSeqnro << "assigned for request(" << command << ")";
    ++m_currentSeqnro;
...
void PdbDebuggerInstance::process()
{
    ...
    if (itr != m_callbacks.end()) {
        if (itr.value()) {
            ...
            // Invoke the method.
            itr.value()(std::as_const(response));
        }
        m_callbacks.erase(itr);
    } else {
