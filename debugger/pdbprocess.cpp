...
PdbProcess::PdbProcess(QObject* parent)
    : QObject(parent)
    , m_debuggerDir(QStandardPaths::locate(QStandardPaths::GenericDataLocation,
                                           QStringLiteral("kdevpythonsupport/debugger/"),
...
void PdbProcess::initialize(QString interpreter, QStringList arguments, QString workingdir, QProcessEnvironment env)
{
    if (!m_socketdir.isValid() || arguments.isEmpty() || m_debuggerDir.isEmpty()) {
        Q_EMIT finished();
        return;
    }
    ...
    *m_debuggerProcess << interpreter << QStringLiteral("-u") // Need unbuffered stdout/stderr
                       << m_debuggerDir + QStringLiteral("kdevpdb.py") << QStringLiteral("-d") << m_debuggerDir
                       << QStringLiteral("-s") << m_socketdir.path() + QStringLiteral("/") << QStringLiteral("--")
                       << arguments;
    m_debuggerProcess->setOutputChannelMode(KProcess::SeparateChannels);
    m_debuggerProcess->setWorkingDirectory(workingdir);
